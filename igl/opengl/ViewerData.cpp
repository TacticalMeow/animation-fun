...
bool igl::opengl::ViewerData::collapse_shape_edges(float collapse_percentage)
{
    ...
    if (!Q.empty())
    {
        ...
        {
            Eigen::MatrixXd V_temp = Eigen::MatrixXd(V);
            Eigen::MatrixXi F_temp = Eigen::MatrixXi(F);
            clear();
            set_mesh(V_temp, F_temp);
            image_texture("E:/Repositories/Animation_igl_engine_build/bin/textures/snake1.png");
            //set_face_based(true);
        }
    }
    return false;
