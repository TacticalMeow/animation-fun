...
//Start of collapse functions region (Eden Mansdorf).

IGL_INLINE bool our_qslim_collapse(
    const int e,
...
{
    int e1, e2, f1, f2;
    return our_qslim_collapse(e, p, V, F, E, EMAP, EF, EI, e1, e2, f1, f2, Qs);
}
    ...
    std::vector<Eigen::Matrix4d>& Qs)
{
    ...
    {
        collapsed = our_qslim_collapse(e, C.row(e), V, F, E, EMAP, EF, EI, e1, e2, f1, f2, Qs);
    }
    else
    {
...
//End of collapse region (Eden Mansdorf).
...
//edge Decimation (Eden Mansdorf)
IGL_INLINE void min_edge_qslim(
    const int e,
    const Eigen::MatrixXd& V,
    const Eigen::MatrixXi& /*F*/,
    const Eigen::MatrixXi& E,
...
void igl::opengl::ViewerData::set_original_vf(const Eigen::MatrixXd& V, const Eigen::MatrixXi& F)
{
    OF = Eigen::MatrixXi(F);
    OV = Eigen::MatrixXd(V);
...
bool igl::opengl::ViewerData::collapse_shape_edges(float collapse_percentage)
{
    if (!Q.empty())
    {
        ...
        // collapse edge
        if (collapse_percentage > 0.5f || collapse_percentage <= 0.0f)
            return false;
...
void igl::opengl::ViewerData::decimate_edges()
{
    Eigen::MatrixXd U;
    Eigen::MatrixXi G;
    Eigen::VectorXi J;
    if (igl::decimate(V, F, 1000, U, G, J))
    {
        clear();
        set_mesh(U, G);
    }
